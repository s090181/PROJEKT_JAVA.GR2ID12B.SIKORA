...
/**
 * klasa conroller pozwalajaca na zarzadzanie widokiem addCar.fxml
 */
public class AddCarController {
    ...
    /**
     * metoda pozwalajaca na dodanie nowego samochodu do bazy danych
     * wymagana jest etapowa walidacja, w przypadku niepoprawnosci etapu zwracana jest informacja o bledzie
     * w celu dodania samochodu do bazy konieczna jest 100% poprawnosc danych
     */
    public void add() {
        ...
        try {
            if (carService.isFieldsValid(map)) {
                ...
                Car car = carService.add(map, fuelType);
                if (car != null) {
                    dBS.insertCar(car);
                    responseMessage("Samochód dodany");
                }
            } else {
